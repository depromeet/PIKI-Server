...
이번 PR 의 URL · 번호 · 브랜치명 · 연관 이슈 번호·제목 · `$ISSUE_LABELS` · PR 제목 · **이번 작업이 무엇인지 한 줄 설명**, 그리고 이번 대화 맥락. (한 줄 설명은 3단계 확인 문구에 그대로 쓴다.)
...
- **확인 문구에는 항상 이번 작업이 무엇인지 한 줄 설명을 먼저 붙인다.** 카드명만 들이밀면 사용자가 "그게 무슨 PR이었지" 를 되짚어야 하므로, `[작업] <한 줄 설명>` 을 앞세운 뒤 카드를 제안한다.
- 사용자에게 제안하고 확인받는다: **"[작업] '<한 줄 설명>' — 이 PR을 '<카드명>' 에 기록할게, 맞아?"** (애매하면 상위 2~3개 제시). `$ISSUE_LABELS` 가 `chore` / `test` / `infra` / `docs` / `refactor` 면 **"이건 <라벨> 라 보통 보드 비대상인데, 그래도 기록할까?"** 처럼 그 신호를 확인 문구에 덧붙여 사용자가 판단하게 한다.

...
- **머지 직전에 timestamp 를 재발급한다.** `outOfOrder=false` (Flyway 기본) 정책상 이미 운영에 더 큰 timestamp 가 들어가 있으면 작업 중인 PR 의 옛 timestamp 가 거부된다. 머지 직전에 `dev` 의 최신 timestamp 를 확인하고, 자기 마이그레이션의 timestamp 를 그 이후로 재발급. 동시에 여러 PR 이 머지 대기 중이면, 먼저 머지된 PR 의 완료를 기다린 뒤 `dev` 의 최신 timestamp 를 다시 확인해 재발급한다 (둘 다 같은 값으로 재발급하면 나중 PR 이 충돌·거부된다).

...
**도구**: Flyway. **위치**: `src/main/resources/db/migration/`. **네이밍**: `V{YYYYMMDDhhmmss}__{snake_case_description}.sql` (예: `V20260521143015__add_index_on_wishes_user_id.sql`). KST 기준, 파일을 만들 때의 현 시각을 prefix 로 부여한다 (`date +%Y%m%d%H%M%S`).

### 규칙

- **이미 적용된 마이그레이션 파일은 수정하지 않는다.** Flyway 는 적용 시점에 checksum 을 저장하고, 이후 파일 내용이 바뀌면 다음 부팅에서 실패한다. 컬럼·제약을 바꿔야 하면 **새 timestamp 로 추가 마이그레이션** 을 작성한다.
- **머지 직전에 timestamp 를 재발급한다.** `outOfOrder=false` (Flyway 기본) 정책상 이미 운영에 더 큰 timestamp 가 들어가 있으면 작업 중인 PR 의 옛 timestamp 가 거부된다. 머지 직전에 main 의 최신 timestamp 를 확인하고, 자기 마이그레이션의 timestamp 를 그 이후로 재발급.

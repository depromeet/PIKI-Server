...
// item(정체성)의 한 추출 버전. item 이 갱신될 때마다 새 version 행이 쌓여 가격·이름·이미지 이력을 보존한다.
...
class ItemSnapshot(
    ...
    // 그 순간 필드는 아직 주입 전이라 전부 null/기본값이다. 그래서 "version >= 1" 같은 필수값·상태-의존 불변식은
    // 여기 두지 않는다(두면 행 하이드레이션이 깨진다) — 그 검증은 생성 경로(후속 단계의 팩토리)가 진다. Item 과 동일 전략.
    init {

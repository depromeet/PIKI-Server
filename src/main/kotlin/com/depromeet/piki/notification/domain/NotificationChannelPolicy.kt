...
// 모든 알림은 인앱 채널(SSE)로 받으므로 타입별로 갈리는 건 "FCM 푸시까지 보내나" 하나뿐 — 그 한 가지만 여기서 정한다.
//   - 아이템 추가/삭제 -> false. 인앱 SSE 로 즉시 반영되므로 OS 트레이 푸시는 노이즈다.
//   - 그 외(남의 행동·내 작업 결과·공지) -> true. 앱이 닫혀 있어도 알려야 한다.
// 이 구분은 런타임 presence(온라인/포그라운드)가 아니라 타입 단위 정적 결정이다. 온라인 유저가 트레이 푸시를

...
// 안읽음 합산 규칙의 단일 소스. REST 읽음 응답·SSE 인앱 배지·OS 아이콘 badge 가 모두 이걸 거쳐 같은 수를 가리키게 한다(drift 방지).
fun Map<NotificationCategory, Long>.toUnreadTotal(): Long = values.sum()

// OS 아이콘 badge(안읽음 수). iOS aps.badge·Android setNotificationCount 가 Int 라 toInt() — 안읽음 수는 Int 범위를 넘지 않는다.
fun Map<NotificationCategory, Long>.toBadgeCount(): Int = toUnreadTotal().toInt()

...
// Apple 웹 form_post 콜백 브릿지(#430). Apple 의 POST(form_post)를 받아 프론트 공용 콜백으로 GET 쿼리 303 하는지,
// 로그인은 하지 않고 code·state 를 그대로 넘기는지(state 미소비) 검증한다. POST→GET 전환이라 302 아닌 303 See Other.
...
class AppleCallbackBridgeIntegrationTest : IntegrationTestSupport() {
    ...
    @Test
    fun `Apple form_post 콜백은 인증 없이 code·state 를 프론트 공용 콜백으로 303 한다`() {
        mockMvc()
            .perform(
    ...
    @Test
    fun `Apple 이 error 를 form_post 로 보내면 프론트 공용 콜백으로 error 를 실어 303 한다`() {
        mockMvc()
            .perform(
                post("/api/v1/auth/apple/callback")
                    .contentType(MediaType.APPLICATION_FORM_URLENCODED)

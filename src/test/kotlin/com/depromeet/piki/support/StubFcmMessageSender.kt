...
// 매 테스트가 본문에서 onSend / onSendBadgeSync 람다를 명시적으로 세팅한다 (셋업 hook · default 리셋 금지).
//
// default 람다는 throw 다. 명시 세팅을 빠뜨리면 즉시 IllegalStateException 으로 깨져
// "이전 테스트의 람다가 살아남아 다음 테스트에 영향을 주는" 함정을 차단한다.
// (FCM 토큰이 없는 대다수 통합 테스트는 PushNotificationChannel 이 토큰 조회 후 early return 하므로
//  이 stub 에 도달하지 않는다 — 토큰을 등록한 발송 검증 테스트만 람다를 세팅한다.)
// 호출 여부·전달 토큰·badge 검증은 테스트 본문에서 람다로 캡처한다(공유 인스턴스 누수를 피해 stub 에 카운터를
...
class StubFcmMessageSender : FcmMessageSender {
    ...
    // 생기면 직접 FcmSendResult 를 돌려주도록 확장한다. badge(#487)는 발송에 실린 안읽음 수 — 테스트가 캡처해 단언한다.
    var onSend: (List<String>, Notification, Int) -> List<String> = { _, _, _ ->
        error("stub.onSend 를 테스트 본문에서 명시 세팅해야 한다. CLAUDE.md '테스트 셋업 원칙' 참고.")
    }

    // 읽음 후 silent badge 동기화(#487) 발송용 람다. badge 는 갱신된 안읽음 수.
    var onSendBadgeSync: (List<String>, Int) -> List<String> = { _, _ ->
        error("stub.onSendBadgeSync 를 테스트 본문에서 명시 세팅해야 한다. CLAUDE.md '테스트 셋업 원칙' 참고.")
    }

...
// 이미지 추출 외부 호출(Gemini Vision)을 통합 테스트에서 격리하기 위한 stub.
...
class StubProductImageExtractor : ProductImageExtractor {
    var build: (ProductImage) -> ImageExtraction = {
        error("stub.build 를 테스트 본문에서 명시 세팅해야 한다. CLAUDE.md '테스트 셋업 원칙' 참고.")
    }

    override fun extract(image: ProductImage): ImageExtraction = build(image)
}
